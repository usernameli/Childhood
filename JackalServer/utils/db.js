...
exports.create_account = function(nickName,avatarUrl,gender,province,city,country,openid,account,callback){
    callback = callback == null? nop:callback;
    ...
    var sql = 'INSERT INTO accounts(nickName,avatarUrl,gender,province,city,country,openid,account,hp) VALUES("' + nickName + '","' + avatarUrl + '","' + gender +'","' + province + '","'+ city +'","' + country + '","' + openid + '","' + account +'","'+ "10" + '")';
    query(sql, function(err, rows, fields) {
        if (err) {
...
exports.is_user_exist = function(account,callback){
    ...
    var sql = 'SELECT userid FROM accounts WHERE account = "' + account + '"';
    query(sql, function(err, rows, fields) {
        if (err) {
            throw err;
        }
...
exports.get_user_hp = function (account,callback) {
    callback = callback == null? nop:callback;
    if(account == null){
        callback(null);
        return;
    ...
    var sql = 'SELECT hp FROM accounts WHERE account = "' + account + '"';
    query(sql, function(err, rows, fields) {
        if (err) {
            callback(null);
            throw err;
