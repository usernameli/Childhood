...
cc.Class({
    ...
    onLoad: function()
    {
        ...
        // scheduler.schedule(this.reportPos, this,0.1, cc.macro.REPEAT_FOREVER);
    ...
    {
        var x = Math.cos(this._angle * (Math.PI/180)) * this._speed;
        var y = Math.sin(this._angle * (Math.PI/180)) * this._speed;
        var  angle = parseInt(this._angle * -1);
        console.log("_allDirectionsMove: " + angle);
        ...
        else
        {
            ...
            {
                angle = (90 - (angle - 90) + 90) * -1  ;
            }
        }
        cc.director.GlobalEvent.emit("set_pos_fly",
            {
                posX: x,
                posY: y,
                angle:angle
